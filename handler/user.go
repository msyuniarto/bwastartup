...
func (h *userHandler) Login(c *gin.Context) {
	...
	if err != nil {
		...
		response := helper.APIResponse("Login failed", http.StatusUnprocessableEntity, "error", errorMessage)
		c.JSON(http.StatusUnprocessableEntity, response)
		return // agar stop disini
	}

	formatter := user.FormatUser(logginUser, "tokentokentokentokentoken")
...
func (h *userHandler) UploadAvatar(c *gin.Context) {
	// input dari user (berupa form body) -> tidak perlu buat struct
	// simpan gambar di folder "images/"
	// di service kita panggil repo
	// JWT -> user login ambil ID
	// repo ambil data user sesuai ID yg didapat dari proses JWT
	//  repo update data user (simpan lokasi file)

	file, err := c.FormFile("avatar") // avatar -> nama parameter
	if err != nil {
